// src/pages/Documents/hooks/useInvoiceData.js
...
export const useInvoiceData = ({
  ...
  const fetchInvoiceTickets = async () => {
    ...
    try {
      ...
      // เฉพาะ booking ที่มี PO Number แล้วเท่านั้น
      let query = supabase
      ...
      `
        ...
        .not("po_number", "is", null) // เฉพาะที่มี PO Number
        .neq("po_number", ""); // และ PO Number ไม่ใช่ string ว่าง
      ...
      if (ticketError) {
        console.error("Error fetching invoice tickets:", ticketError);
      ...
      passengers?.forEach((passenger) => {
        ...
        passengersMap
          .get(passenger.bookings_ticket_id)
          .push(passenger.passenger_name);
      });
      ...
      routes?.forEach((route) => {
        ...
        routesMap
          .get(route.bookings_ticket_id)
          .push(`${route.origin}-${route.destination}`);
      });
      ...
      const processedData = adjustedTickets.map((ticket) => {
        ...
        const firstPassengerTicketInfo =
          firstPassengerTicketMap.get(ticket.id) || null;

        const passengersDisplay =
          ticketPassengers.length > 0
            ? ticketPassengers.length === 1
              ? ticketPassengers[0]
              : `${ticketPassengers[0]} +${ticketPassengers.length - 1} อื่นๆ`
            : "-";

        const routingDisplay =
          ticketRoutes.length > 0
            ? ticketRoutes.length === 1
              ? ticketRoutes[0]
              : `${ticketRoutes[0]} +${ticketRoutes.length - 1}`
            : "-";
